...
export const CandidateProfileSchema = z.object({
  ...
  aspirations: z.object({
    suggestionType: z.string().nullable().optional().describe('The type of job suggestion (e.g., "accurate", "related").'),
    workLocation: z.array(z.string()).nullable().optional().describe('The desired work location (e.g., "Osaka").'),
    career: z.string().nullable().optional().describe('The desired work location (e.g., "Osaka").'),
    job: z.string().nullable().optional().describe('The desired work location (e.g., "Osaka").'),
    basicSalary: z.number().min(0).max(1000000).nullable().optional().describe('The desired basic salary (e.g., "180,000 yên").'),
    realSalary: z.number().min(0).max(1000000).nullable().optional().describe('The desired net salary (e.g., "160,000 yên").'),
    visa: z.string().optional().describe('The detailed desired visa type (e.g., "Đặc định đầu Nhật").'),
    visaDetail: z.string().nullable().optional().describe('The detailed desired visa type (e.g., "Đặc định đầu Nhật").'),
    fee: z.number().min(0).max(3800).nullable().optional().describe('Financial ability (e.g., "90 triệu").'),
    interviewLocation: z.string().nullable().optional().describe('The location for job interviews (e.g., "Hà Nội").'),
    interviewDate: z.number().nullable().optional().describe('Thời gian phỏng vấn mong muốn'),
    interviewDateType: z.string().nullable().optional().describe('A list of special requests or aspirations (e.g., ["Tăng ca", "Hỗ trợ nhà ở"]).'),
    gender: z.string().optional().nullable().describe('Giới tính'),
    haveTattoo: z.string().nullable().optional().describe('Hình xăm'),
  }).optional(),
